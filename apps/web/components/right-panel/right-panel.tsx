...
export function RightPanel() {
  ...
  // Get template values from store
  ...
  const handleTemplateChange = (templateValue: string) => {
    ...
    if (template) {
      ...
      localStorage.setItem(latexKey, template.latex);
  ...
  useEffect(() => {
    ...
    if (!currentContent && currentTemplate) {
      ...
      localStorage.setItem(latexKey, currentTemplate.latex);
    }
  }, [
    rightPanelCategory,
    currentTemplate,
  ...
  const handleReset = () => {
    try {
      // Get the template value from store based on document type
      const templateValue =
        rightPanelCategory === navigation.RIGHT_PANEL.RESUME
          ? resumeTemplate
          : coverLetterTemplate;
      ...
      // Use the original template's LaTeX content from template config
      ...
      localStorage.setItem(latexKey, defaultTemplate.latex);
