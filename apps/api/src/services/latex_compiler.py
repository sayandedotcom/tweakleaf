...
class LatexCompilerService:
    ...
    def copy_required_files(self, temp_path: Path) -> None:
        ...
        # Copy the existing cover.cls file to the temporary directory
        source_cls = Path("src/latex/cover.cls")
        cls_file = temp_path / "cover.cls"
        
        if not source_cls.exists():
            raise HTTPException(status_code=500, detail="cover.cls file not found in working directory")
        
        shutil.copy2(source_cls, cls_file)
        logger.info(f"Copied cover.cls to: {cls_file}")
        ...
        if source_fonts.exists():
            ...
            logger.info(f"Copied OpenFonts directory to: {fonts_dest}")
